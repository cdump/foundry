...
impl SignaturesIdentifier {
    async fn identify<T>(
        ...
        let hex_identifiers: Vec<String> = identifiers.iter().map(hex::encode_prefixed).collect();
    ...
    pub async fn identify_function(&mut self, identifier: &impl AsRef<[u8]>) -> Option<Function> {
    ...
    pub async fn identify_event(&mut self, identifier: &impl AsRef<[u8]>) -> Option<Event> {
