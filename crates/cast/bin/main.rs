...
async fn main() -> Result<()> {
    ...
    match opts.sub {
        ...
        Subcommands::Selectors { bytecode, resolve } => {
            ...
            if resolve {
                let selectors: Vec<_> = selectors_and_args.iter().map(|r| &r.0).collect();
                let max_args_len = selectors_and_args.iter().map(|r| r.1.len()).max().unwrap_or(0);

                let resolve_results = decode_selectors(&selectors, SelectorType::Function).await?;
                ...
                for (selector, arguments) in selectors_and_args.iter() {
